  ...
  {#if showAuthDropdown}
    <div class="modal-backdrop">
      <button
        ...
        type="button"
        aria-label="Close dialog"
        onclick={() => { showAuthDropdown = false; emailSent = false; emailError = ''; emailInput = '' }}
      ></button>
      {#if $user}
      ...
      {:else}
        <div class="auth-modal" role="dialog" aria-modal="true">
          <p class="auth-modal-title">ANUKA UCHIKA</p>
          ...
          {#if emailSent}
            <p class="auth-email-sent">Check your email for the login link</p>
          {:else}
            <form class="auth-email-form" onsubmit={(e) => {
              e.preventDefault()
              emailError = ''
              ...
              <input type="email" class="auth-email-input" bind:value={emailInput} placeholder="Email" required use:focus />
              {#if emailError}<p class="auth-email-error">{emailError}</p>{/if}
              <button type="submit" class="auth-modal-btn">Send Sign In Link</button>
            </form>
          {/if}
        </div>
      {/if}

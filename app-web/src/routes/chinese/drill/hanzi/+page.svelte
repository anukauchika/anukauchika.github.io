<script>
  ...
  $effect(() => {
    if (sttDataset.id && drillGroup) {
      svcDrill.initSession(sttDataset.id, 'stroke', drillGroup.id, drillGroup.items)
    }
  })
</script>
...
<main class="anuka-page">
  {#if sttDrill.currentItem && !sttDrill.sessionDone}
    <Island>
      ...
      {#if sttAuth.isAuthenticated && sttDrill.currentStat}
        <span class="anuka-badge anuka-main">{sttDrill.currentStat.successCount}{#if sttDrill.currentStat.errorCount > 0}<span class="anuka-fail">| {sttDrill.currentStat.errorCount}</span>{/if}</span>
      {/if}
      <DrillStroke />
    </Island>
  {/if}
  ...
  <ProgressLine fill={sttDrill.progress}>
    {#snippet bottom()}<div class="anuka-row anuka-center"><span class="anuka-mute anuka-sm">{sttDrill.currentIndex + 1} / {sttDrill.items.length}</span></div>{/snippet}
  </ProgressLine>
  ...
  <div class="anuka-tags anuka-center">
    {#each sttDrill.items as item, idx}
      ...
      <span
        class="anuka-tag"
        class:anuka-main={idx === sttDrill.currentIndex}
        class:anuka-succ={sttDrill.completedWords.has(idx)}
        title={item.word}
      >
        {item.tr}

-- EXECUTE ... USING still produced the same slow parameterized plan as the
-- original SQL function. Render typed SQL literals with format(%L) so the
-- planner sees the actual dataset, group-array cardinality, and timezone.
-- %L quotes and escapes values as SQL literals; it does not allow SQL injection.
...
begin
  return query execute format($query$
    with group_list as (
      ...
      from unnest(%1$L::integer[]) with ordinality as g(group_id, ord)
    ),
    practice_types as (
    ...
    select
      ...
      case
        ...
        when (d.computed_due_at at time zone %3$L::text)::date
          <= (now() at time zone %3$L::text)::date then 'due'
        else 'upcoming'
      end as reason,
    ...
    from due_times d
  $query$, p_group_ids, p_dataset_id, p_timezone);
end;
$function$;

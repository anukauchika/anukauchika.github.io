create index if not exists idx_group_session_next_chinese_drill
  on public.group_session(user_id, dataset_id, group_id, practice_type, done_at desc)
...
as $$
  ...
  candidates as (
    ...
    select
      ...
      r.practice_type,
      'due'::text as reason,
      r.last_clean_at + make_interval(days => r.interval_days) as due_at,
      r.interval_days,
      1 as rank,
      extract(epoch from r.last_clean_at + make_interval(days => r.interval_days)) as primary_order,
      r.ord as secondary_order,
      r.type_ord as tertiary_order
    from reviews r
    where r.full_count > 0
      ...
      and r.last_clean_at + make_interval(days => r.interval_days) <= now()

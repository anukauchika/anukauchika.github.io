-- One-round-trip summary for the app main page: today's activity, due count,
-- drilled words total and the next suggested drill. Reuses
-- chinese_group_review_state for the SRS aggregation (single pass) and
-- inlines the ranking previously in next_chinese_drill over the shared result.
-- Purely additive — safe to apply while the previous frontend is still live.
-- Supersedes chinese_day_progress and next_chinese_drill, which are left in
-- place for now; stale db entities are to be reviewed and dropped separately.
...
as $$
  ...
  today as (
    select
      ...
      coalesce(sum(
        least(
          greatest(extract(epoch from gs.done_at - gs.started_at) * 1000, 0),
          7200000
        )
...
revoke execute on function public.chinese_home_summary(text, integer[], text) from public;
revoke execute on function public.chinese_home_summary(text, integer[], text) from anon;
grant execute on function public.chinese_home_summary(text, integer[], text) to authenticated;

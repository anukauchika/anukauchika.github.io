...
as $$
  ...
  select
    ...
    case
      ...
      when (d.computed_due_at at time zone p_timezone)::date <= (now() at time zone p_timezone)::date then 'due'
      else 'upcoming'
    end as reason,
...
as $$
  select *
  from public.chinese_group_review_state(p_dataset_id, p_group_ids, 'UTC');
$$;
...
as $$
  select *
  from public.next_chinese_drill(p_dataset_id, p_group_ids, 'UTC');
$$;

revoke execute on function public.chinese_group_review_state(text, integer[], text) from public;
revoke execute on function public.chinese_group_review_state(text, integer[], text) from anon;
grant execute on function public.chinese_group_review_state(text, integer[], text) to authenticated;

revoke execute on function public.next_chinese_drill(text, integer[], text) from public;
revoke execute on function public.next_chinese_drill(text, integer[], text) from anon;
grant execute on function public.next_chinese_drill(text, integer[], text) to authenticated;
